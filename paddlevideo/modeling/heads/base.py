    ...
    def loss(self, scores, labels, valid_mode=False, **kwargs):
        ...
        if len(labels) == 1:  #commonly case
            ...
            if self.ls_eps != 0. and not valid_mode:  # label_smooth
                ...
                loss = self.loss_func(scores, labels, valid_mode=valid_mode, **kwargs)
            ...
            scores1 = scores[:bs//2]
            scores2 = scores[bs//2:]
            p = nn.functional.log_softmax(scores1.reshape((-1, self.num_classes)), axis=-1)
            p_tec = nn.functional.softmax(scores1.reshape((-1, self.num_classes)), axis=-1)
            q = nn.functional.log_softmax(scores2.reshape((-1, self.num_classes)), axis=-1)
            q_tec = nn.functional.softmax(scores2.reshape((-1, self.num_classes)), axis=-1)
            ...
            #loss = loss + 0.3*(kl_loss + reverse_kl_loss)
