...
class CustomWarmupMultiStepDecay(LinearWarmup):
    def __init__(self, warmup_epochs, start_lr=0., end_lr=None, base_lr=None, milestones=None, gamma=0.1, last_epoch=-1, verbose=False):
        #super().__init__()
        if end_lr is None and base_lr is not None:
            end_lr = base_lr
        elif end_lr is not None and base_lr is None:
            base_lr = end_lr
        elif end_lr is None and base_lr is None:
            raise ValueError("The end_lr and base_lr can not be both None!")

        lr = MultiStepDecay(learning_rate=base_lr, milestones=milestones, gamma=gamma, last_epoch=last_epoch, verbose=verbose)
        super(CustomWarmupMultiStepDecay, self).__init__(learning_rate=lr, warmup_steps=warmup_epochs, start_lr=start_lr, end_lr=end_lr, last_epoch=last_epoch, verbose=verbose)
