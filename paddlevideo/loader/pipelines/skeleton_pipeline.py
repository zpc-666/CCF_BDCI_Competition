...
class AutoPadding(object):
    ...
    """
    def __init__(self, window_size, random_pad=False, mode="joint"):
    ...
    def __call__(self, results):
        data = results['data']
        #T1 = self.get_frame_num(data)
        if self.mode=="bone":
            data = get_bone(data)
        elif self.mode=="joint_motion":
            data = get_joint_motion(data)
        elif self.mode=="bone_motion":
            data = get_bone(data)
            data = get_joint_motion(data)
            #print(data.shape)
        C, T, V, M = data.shape
